...
- 设计 GitHub Actions CI/CD，自动执行 FastAPI 单测、70 道评测数据校验、凭据扫描和受限容器健康检查；通过受保护分支与 Environment 人工审批发布小型源码包，生产机先构建候选镜像，再经健康门禁切换并支持旧版本自动回滚；定位并解决海外 Runner 直传镜像导致的跨境链路超时。
